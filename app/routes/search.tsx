...
export const createAccordionContent = (providerResult: ProviderResult) => {
    ...
    (<>We found {providerResult.products.length} products matching your search criteria at {providerResult.providerName}. You can search in provider's website by clicking{" "}<a className="underline" href={providerResult.queryUrl} target="_blank" rel="noreferrer">here</a></>) 
    : (<>We haven't found any results for your search. This can be due to the search term not being found in their website or them blocking us. Nevertheless you can still check in their website by clicking{" "}<a className="underline" href={providerResult.queryUrl} target="_blank" rel="noreferrer">here</a></>)
            ...
            <form method="get" action="/search">
              <div className="flex pt-10 px-48 w-full">
                <Input
                  name="q"
                  type="text"
                  placeholder="Find something different..."
                  required
                />
              </div>
            </form>
        ...
        <Accordion type="multiple" className="w-full">
          {providerResults.map((providerResult) => {
            return (
              ...
              >
                <AccordionTrigger>
                  {providerResult.providerName} - Found {providerResult.products.length} products
                </AccordionTrigger>
                <AccordionContent>{createAccordionContent(providerResult)}</AccordionContent>
              </AccordionItem>
            );
          })}
        </Accordion>
